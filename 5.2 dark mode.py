...
# Определение функции для изменения темы
def change_theme():
    if theme_var.get() == "dark":
        colors = {
            ...
            "fg": "white",
            "button_bg": "#333333",
            "select_bg": "grey",   # Цвет фона выбранного чекбокса на темном фоне
            "select_fg": "white"   # Цвет текста выбранного чекбокса на темном фоне
        }
    elif  theme_var.get() == "light":
        colors = {
            "bg": "white",
            "fg": "black",
            "button_bg": "#4CAF50",
            "select_bg": "white",  # Цвет фона выбранного чекбокса
            "select_fg": "black"   # Цвет текста выбранного чекбокса
        }

    root.config(bg=colors["bg"])
    output_field.config(bg=colors["bg"], fg=colors["fg"], insertbackground=colors["fg"])  # insertbackground изменяет цвет курсора

    # Обновляем цвет фона и текста всех меток, полей ввода и кнопок
    for widget in root.winfo_children():
        ...
        if isinstance(widget, Checkbutton):
            widget.config(bg=colors["bg"], fg=colors["fg"], selectcolor=colors["select_bg"], activebackground=colors["bg"], activeforeground=colors["fg"])
   # Обновляем цвет чекбоксов
    npt_checkbox.config(bg=colors["bg"], selectcolor=colors["select_bg"], activebackground=colors["bg"])
    wheil_checkbox.config(bg=colors["bg"], selectcolor=colors["select_bg"], activebackground=colors["bg"])
    albacore_checkbox.config(bg=colors["bg"], selectcolor=colors["select_bg"], activebackground=colors["bg"])

# Добавление меню
menu_bar = Menu(root)
root.config(menu=menu_bar)

# Добавление пункта меню "Настройки"
settings_menu = Menu(menu_bar, tearoff=0)
menu_bar.add_cascade(label="Настройки", menu=settings_menu)

# Добавление пункта меню "Тема"
theme_var = StringVar(root)
theme_var.set("dark")  # по умолчанию
theme_menu = Menu(settings_menu, tearoff=0)
settings_menu.add_cascade(label="Тема", menu=theme_menu)

# Добавление пунктов в меню "Тема"
light_theme = Menu(theme_menu, tearoff=0)
theme_menu.add_radiobutton(label="Светлая", variable=theme_var, value="light", command=change_theme)

dark_theme = Menu(theme_menu, tearoff=0)
theme_menu.add_radiobutton(label="Темная", variable=theme_var, value="dark", command=change_theme)
...
Button(root, text="Копировать", command=copy_info, font=("Arial", 14), fg="black").grid(row=3, column=0, sticky=W, padx=10, pady=10)


# Добавление кнопки для генерации информации
Button(root, text="Сгенерировать", command=generate_info, font=("Arial", 14), fg="black").grid(row=3, column=1, sticky=E, padx=10, pady=10)
...
# Установка темы по умолчанию и вызов функции для ее применения
theme_var.set("dark")
change_theme()  # применить тему "dark" сразу после старта программы
